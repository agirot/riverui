...
type EndpointMeta struct {
	// Path is the API endpoint's path where it should be mounted, which is
	// passed to http.ServeMux by Mount. It should start with a verb like `GET`
...
func Mount[TReq any, TResp any](mux *http.ServeMux, logger *slog.Logger, apiEndpoint EndpointInterface[TReq, TResp]) {
	...
	mux.HandleFunc(meta.Path, func(w http.ResponseWriter, r *http.Request) {
